...
/**
 * This is the domain object that represents a Document in our database. Since this is a document
 * based database, this is the smallest unit of data storage. In other words, this object represents
 * a single record in Docussandra.
 *
...
public class Document extends Timestamped implements Identifiable {
  ...
  // https://github.com/PearsonEducation/Docussandra/issues/7
  // TODO: add any necessary metadata regarding a document.
  // https://github.com/PearsonEducation/Docussandra/issues/8
  ...
  @ApiModelProperty(value = "The table's name",
      notes = "This is the table name in which this document is stored.", dataType = "String",
      required = true)
  @Required("Table")
  @ChildValidation
  ...
  /**
   * The actual JSON document we are storing. Stored as BSON. This is the record itself, with no
   * additional Docussandra metadata.
   */
  ...
  /**
   * Gets the unique identifier for this object inside of the given db and table.
   *
   * @return
   */
  public UUID getUuid() {
  ...
  /**
   * Returns true if there is a table associated with this object. This should nearly always be the
   * case, unless the object was constructed improperly or incompletely. Private for that reason;
   * used internally for null checks.
   *
   * @return
   */
   ...
   *
   * @param database Name of the database that we are storing this document in.
   * @param table Name of the table (inside of the database) that we are storing this document in.
   */
  ...
  /**
   * Gets the object stored in this document object, as a String. Note, this requires deserializing
   * the BSON, so if you can work with the BSON directly, use the getObject() call instead, it will
   * perform better.
   *
  ...
  /**
   * Sets the actual document (record) that is stored in Docussandra, as a String. Note this
   * requires serializing the object to BSON, so if you already have the object as BSON, you should
   * call setObject() instead, as it will perform better.
   *
   * @param json
   */
