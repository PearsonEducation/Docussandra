...
public class QueryController {
  ...
  public Document[] query(Request request, Response response) throws IndexParseException {
    ...
    toQuery.setTable(table);// change of plans, no longer getting it from the query object, but from
                            // the URL instead

    QueryRange range = QueryRanges.parseFrom(request);

    int limit = DEFAULT_LIMIT;// TODO: enforce maximum limit
                              // https://github.com/PearsonEducation/Docussandra/issues/14
    long offset = 0;
    if (range != null) {
      if (range.hasLimit()) {
    ...
    try {
      ...
      } else if (queryResponse.getNumAdditionalResults() == null) {// we have more results, but an
                                                                   // unknown number more
        response.setCollectionResponse(range, queryResponse.size(), -1);
        response.setResponseStatus(HttpResponseStatus.PARTIAL_CONTENT);
      } else if (queryResponse.getNumAdditionalResults() == 0l) {// we have no more results, the
                                                                 // amount returned is the number
                                                                 // that exists
        response.setCollectionResponse(range, queryResponse.size(), queryResponse.size());
      } else// not likely to actually happen given our implementation, but coding in case our
            // backend changes
      {
        response.setCollectionResponse(range, queryResponse.size(),
            queryResponse.size() + queryResponse.getNumAdditionalResults());
      }
      logger.debug(
          "Query: " + toQuery.toString() + " returned " + queryResponse.size() + " documents.");
      if (returnHAL) {
