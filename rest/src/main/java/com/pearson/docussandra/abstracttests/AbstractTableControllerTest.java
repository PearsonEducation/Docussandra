...
/**
 * Tests for the TableController class. ROUTE : /databases/{database}/tables/{table}
 *
...
public abstract class AbstractTableControllerTest {
  ...
  /**
   * Tests that the GET /databases/{databases}/tables/{table} properly retrieves an existing table.
   */
  @Test
  public void getTableTest() {
    Table testTable = Fixtures.createTestTable();
    f.insertTable(testTable);
    expect().statusCode(200).body("name", equalTo(testTable.getName()))
        .body("description", equalTo(testTable.getDescription())).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().get(testTable.getName());
  }

  /**
   * Tests that the POST /databases/{databases}/tables/{table} endpoint properly creates a table.
   */
  @Test
  public void postTableTest() {
    ...
    given().body(tableStr).expect().statusCode(201)
        ...
        .body("description", equalTo(testTable.getDescription())).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().post("/" + testTable.getName());
    // check
    expect().statusCode(200).body("name", equalTo(testTable.getName()))
        .body("description", equalTo(testTable.getDescription())).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().get(testTable.getName());
  ...
  /**
   * Tests that the PUT /databases/{databases}/tables/{table} endpoint properly updates a table.
   */
  @Test
  public void putTableTest() {
    ...
    String tableStr = "{" + "\"description\" : \"" + newDesciption + "\"," + "\"name\" : \""
        + testTable.getName() + "\"}";

    // act
    given().body(tableStr).expect().statusCode(204).when().put(testTable.getName());
  ...
  public void deleteTableTest() {
    ...
    f.insertTable(testTable);
    // act
    given().expect().statusCode(204).when().delete(testTable.getName());
    // check
    expect().statusCode(404).when().get(testTable.getName());
  }
}
