...
/**
 * Tests for the database controller tests ROUTE: /databases/
 * 
...
public abstract class AbstractDatabaseControllerTest {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(AbstractDatabaseControllerTest.class);
  ...
  public static void beforeClass() throws Exception {
    ...
    /*
     * String testEnv = System.getProperty("TEST_ENV") != null ? System.getProperty("TEST_ENV") :
     * "local"; String[] env = {testEnv}; Thread.sleep(10000);
     */
  ...
  public void getDatabaseTest() {
    Database testDb = Fixtures.createTestDatabase();
    f.insertDatabase(testDb);
    expect().statusCode(200).body("name", equalTo(testDb.getName()))
        .body("description", equalTo(testDb.getDescription())).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().get("/" + testDb.getName());
  }
  ...
  public void postDatabaseTest() {
    Database testDb = Fixtures.createTestDatabase();
    String dbStr = "{" + "\"description\" : \"" + testDb.getDescription() + "\"," + "\"name\" : \""
        + testDb.getName() + "\"}";
    // act
    given().body(dbStr).expect().statusCode(201)
        ...
        .body("description", equalTo(testDb.getDescription())).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().post(testDb.getName());
    // check
    expect().statusCode(200).body("name", equalTo(testDb.getName()))
        .body("description", equalTo(testDb.getDescription())).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().get("/" + testDb.getName());
  ...
  public void putDatabaseTest() {
    ...
    String dbStr = "{" + "\"description\" : \"" + newDesciption + "\"," + "\"name\" : \""
        + testDb.getName() + "\"}";
    // act
    given().body(dbStr).expect().statusCode(204).when().put(testDb.getName());
    // check
    expect().statusCode(200).body("name", equalTo(testDb.getName()))
        .body("description", equalTo(newDesciption)).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().get("/" + testDb.getName());
  ...
  public void deleteDatabaseTest() {
    ...
    f.insertDatabase(testDb);
    // act
    given().expect().statusCode(204).when().delete(testDb.getName());
    // check
    expect().statusCode(404).when().get(testDb.getName());
  }

  /**
   * Tests that the DELETE /{databases} endpoint properly deletes a database and then cascades that
   * delete to any child tables.
   */
  @Test
