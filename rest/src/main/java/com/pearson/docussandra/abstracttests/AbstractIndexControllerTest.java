...
/**
 * Tests for the IndexController. Located in the main test code so it can be implemented outside of
...
public abstract class AbstractIndexControllerTest {
  ...
  public void beforeTest() {
    ...
    RestAssured.basePath =
        "/databases/" + testDb.getName() + "/tables/" + testTable.getName() + "/indexes";
  }
  ...
  /**
   * Tests that the GET /databases/{databases}/tables/{table}/indexes/{index} properly retrieves an
   * existing index.
   */
  @Test
  public void getIndexTest() {
    ...
    ResponseOptions result = expect().statusCode(200).body("name", equalTo(testIndex.getName()))
        .body("fields", notNullValue()).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).when().get("/" + testIndex.getName()).andReturn();

    logger.info(result.body().prettyPrint());
  }

  /**
   * Tests that the POST /{databases}/{setTable}/indexes/ endpoint properly creates a index.
   */
  @Test
  public void postIndexTest() throws InterruptedException {
    ...
    // act
    given().body(indexStr).expect().statusCode(201).body("index.name", equalTo(testIndex.getName()))
        .body("index.fields", notNullValue()).body("index.createdAt", notNullValue())
        .body("index.updatedAt", notNullValue()).body("index.active", equalTo(false))
        .body("id", notNullValue()).body("dateStarted", notNullValue())
        .body("statusLastUpdatedAt", notNullValue()).body("totalRecords", equalTo(0))
        .body("recordsCompleted", equalTo(0)).when().post("/" + testIndex.getName());

    Thread.sleep(100);// sleep for a hair to let the indexing complete

    // check self (index endpoint)
    expect().statusCode(200).body("name", equalTo(testIndex.getName()))
        .body("fields", notNullValue()).body("createdAt", notNullValue())
        .body("updatedAt", notNullValue()).body("active", equalTo(true))
  ...
  public void postIndexAllFieldsTest() throws InterruptedException {
    ...
    given().body(indexStr).expect().statusCode(201).body("index.name", equalTo(testIndex.getName()))
        ...
        .body("index.active", equalTo(false)).body("id", notNullValue())
        .body("dateStarted", notNullValue()).body("statusLastUpdatedAt", notNullValue())
        .body("totalRecords", equalTo(0)).body("recordsCompleted", equalTo(0)).when().log()
        .ifValidationFails().post("/" + testIndex.getName());
    ...
    expect().statusCode(200).body("name", equalTo(testIndex.getName()))
        ...
        .body("active", equalTo(true)).get("/" + testIndex.getName());
  }

  /**
   * Tests that the POST /{databases}/{setTable}/indexes/ endpoint properly creates a index and that
   * the GET/{database}/{setTable}/index_status/{status_id} endpoint is working.
   */
  @Test
  public void postIndexAndCheckStatusTest() throws InterruptedException {
    ...
    ResponseOptions response = given().body(indexStr).expect().statusCode(201)
        ...
        .body("percentComplete", notNullValue()).body("totalRecords", equalTo(0))
    ...
    try {
      ...
      RestAssured.basePath = "/databases/" + testIndex.getDatabaseName() + "/tables/"
          + testIndex.getTableName() + "/index_status/";
      ResponseOptions res = expect().statusCode(200).body("id", equalTo(uuidString))
          .body("dateStarted", notNullValue()).body("statusLastUpdatedAt", notNullValue())
          .body("eta", notNullValue()).body("percentComplete", notNullValue())
  ...
  /**
   * Tests that the POST /databases/{databases}/tables/{table}/indexes endpoint properly creates a
   * index and that the GET/databases/{database}/tables/{table}/index_status/{status_id} endpoint is
   * working.
   */
  @Test
  public void createDataThenPostIndexAndCheckStatusTest() throws InterruptedException, Exception {
    ...
    try {
      ...
      RestAssured.basePath = "/databases/" + lastname.getDatabaseName() + "/tables/"
          + lastname.getTableName() + "/indexes";
      // act -- create index
      ResponseOptions response = given().body(indexStr).expect().statusCode(201)
          .body("index.name", equalTo(lastname.getName())).body("index.fields", notNullValue())
          .body("index.createdAt", notNullValue()).body("index.updatedAt", notNullValue())
          .body("index.active", equalTo(false))// should not yet be active
          .body("id", notNullValue()).body("dateStarted", notNullValue())
          .body("statusLastUpdatedAt", notNullValue()).body("eta", notNullValue())
          .body("percentComplete", notNullValue()).body("totalRecords", equalTo(444))
          .body("recordsCompleted", equalTo(0)).when().post("/" + lastname.getName()).andReturn();
      ...
      String uuidString = response.getBody().jsonPath().get("id");
      RestAssured.basePath = "/databases/" + lastname.getDatabaseName() + "/tables/"
          + lastname.getTableName() + "/index_status/";
      ResponseOptions res = expect().statusCode(200).body("id", equalTo(uuidString))
          .body("dateStarted", notNullValue()).body("statusLastUpdatedAt", notNullValue())
          .body("eta", notNullValue()).body("percentComplete", notNullValue())
      ...
      while (!active) {
        ...
        res = expect().statusCode(200).body("id", equalTo(uuidString))
            .body("dateStarted", notNullValue()).body("statusLastUpdatedAt", notNullValue())
            .body("eta", notNullValue()).body("percentComplete", notNullValue())
            .body("index", notNullValue()).body("index.active", notNullValue())
            .body("recordsCompleted", notNullValue()).when().get(uuidString).andReturn();
  ...
  /**
   * Tests that the POST /databases/{databases}/tables/{table}/indexes endpoint properly creates a
   * index, setting errors to the index status table and that the GET
   * /databases/{database}/tables/{table}/index_status/{status_id} endpoint is working.
   */
  @Test
  public void createBadDataThenPostIndexAndCheckStatusTest() throws Exception {
    ...
    try {
      ...
      RestAssured.basePath = "/databases/" + approvedyear.getDatabaseName() + "/tables/"
          + approvedyear.getTableName() + "/indexes";
      // act -- create index
      ResponseOptions response = given().body(indexString).expect().statusCode(201)
          .body("index.name", equalTo(approvedyear.getName())).body("index.fields", notNullValue())
          .body("index.createdAt", notNullValue()).body("index.updatedAt", notNullValue())
          .body("index.active", equalTo(false))// should not yet be active
          .body("id", notNullValue()).body("dateStarted", notNullValue())
          .body("statusLastUpdatedAt", notNullValue()).body("eta", notNullValue())
          .body("percentComplete", notNullValue()).body("totalRecords", equalTo(444))
          .body("recordsCompleted", equalTo(0)).when().post("/" + approvedyear.getName())
          .andReturn();
      ...
      String uuidString = response.getBody().jsonPath().get("id");
      RestAssured.basePath = "/databases/" + approvedyear.getDatabaseName() + "/tables/"
          + approvedyear.getTableName() + "/index_status/";
      ResponseOptions res = expect().statusCode(200).body("id", equalTo(uuidString))
          .body("dateStarted", notNullValue()).body("statusLastUpdatedAt", notNullValue())
          .body("eta", notNullValue()).body("percentComplete", notNullValue())
          .body("index", notNullValue()).body("index.active", notNullValue())
          .body("index.active", equalTo(false))// should not yet be active -- we are using some
                                               // smaller test data now, so it may have actually
                                               // completed -- if this line errors, up the size of
                                               // the dataset?
          .body("recordsCompleted", notNullValue()).when().get(uuidString).andReturn();
      logger.debug("Status Response: " + res.getBody().prettyPrint());
      ...
      // once it is active, lets check and make sure we have an error in the status table for our
      // bad doc -- side note: there are tons more errors than our intentional one in this dataset
      res = expect().statusCode(200).body("id", equalTo(uuidString))
          .body("dateStarted", notNullValue()).body("statusLastUpdatedAt", notNullValue())
          .body("eta", notNullValue()).body("percentComplete", notNullValue())
          .body("index", notNullValue()).body("index.active", notNullValue())
      ...
      Assert.assertTrue(res.getBody().prettyPrint().contains("52b213b38594d8a2be17c780"));// make
                                                                                          // sure
                                                                                          // our
                                                                                          // (internal)
                                                                                          // doc id
  ...
  /**
   * Tests that the POST /databases/{databases}/tables/{table}/indexes endpoint properly creates a
   * index and that the GET/databases/{database}/tables/{table}/index_status/{index} endpoint is
   * working.
   */
  @Test
  public void postIndexAndCheckStatusAllTest() throws Exception {
    ...
    try {
      ...
      ResponseOptions res =
          expect().statusCode(200).body("_embedded.indexcreatedevents[0].id", notNullValue())
              ...
              .body("_embedded.indexcreatedevents[0].index.active", equalTo(false))// should not yet
                                                                                   // be active
              ...
              .body("_embedded.indexcreatedevents[0].percentComplete", notNullValue()).when().log()
  ...
  /**
   * Tests that the DELETE /databases/{databases}/tables/{table}/indexes/{index} endpoint properly
   * deletes a index.
   */
  @Test
  public void deleteIndexTest() {
    ...
    f.insertIndex(testIndex);
    // act
    given().expect().statusCode(204).when().delete(testIndex.getName());
    // check
    expect().statusCode(404).when().get(testIndex.getName());
  }
