...
public class SecurityPluginPreproccessor implements Postprocessor {
  ...
  public void process(Request request, Response response) {
    ...
    for (SecurityPlugin plugin : securityPlugins) {
      ...
      try {
        plugin.doValidate(headers, request.getPath(),
            HttpMethod.forString(request.getHttpMethod().name()));
  ...
  /**
   * Extracts the headers from a org.restexpress.Request object and returns them as a HashMap
