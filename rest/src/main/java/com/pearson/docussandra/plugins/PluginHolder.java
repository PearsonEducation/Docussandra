...
public class PluginHolder {
  ...
  /**
   * Reads in all the possible plugins from the passed in plugin jars and builds this object.
   * Warning, this is a very expensive operation so it should (and can be) only be called once. Call
   * getInstance() to get a reference to this object after build();
   *
   ...
   * @throws MalformedURLException If there is a problem reading in the plugins.
   * @throws InstantiationException If there is a problem creating a new instance of one of the
   *         plugin classes.
   * @throws IllegalAccessException If we don't have permissions to create a new plugin class
   *         (probably a private constructor issue?).
   */
  public static PluginHolder build(File[] pluginJars)
      throws MalformedURLException, InstantiationException, IllegalAccessException {
    if (instance != null) {
      throw new IllegalStateException(
          "The PluginHolder has already been built. Please call getInstance() Instead.");
    }
    ...
    for (File pluginJar : pluginJars) {
      notifierClasses
          .addAll(PluginUtils.getPluginsFromExternalJar(pluginJar, NotifierPlugin.class));
      securityClasses
          .addAll(PluginUtils.getPluginsFromExternalJar(pluginJar, SecurityPlugin.class));
    }
    for (Class<? extends Plugin> clazz : notifierClasses) {
      NotifierPlugin object = (NotifierPlugin) clazz.newInstance();
      logger.info("Loading Notifier Plugin: " + object.getPluginName() + " of class type: "
    ...
    for (Class<? extends Plugin> clazz : securityClasses) {
      ...
      logger.info("Loading Security Plugin: " + object.getPluginName() + " of class type: "
          + object.getClass().getCanonicalName());
  ...
  /**
   * Gets a reference to this object. Must call build() to build the object before calling this
   * method.
   *
  ...
  public static PluginHolder getInstance() {
    if (instance == null) {
      throw new IllegalStateException(
          "The PluginHolder has not yet been built. Please call build() before calling get.");
    }
    return instance;
  }
