...
public class PluginUtilsTest {
  ...
  /**
   * Tests the getPluginJars() method. We can't really have any presupposed data in the users home
   * dir, so this really isn't much of a test. We will just check to make sure this doesn't error
   * when called and doesn't return null.
   */
  @Test
  ...
  public void testGetPluginsFromExternalJar() throws Exception {
    ...
    ArrayList<Class<? extends Plugin>> result =
        PluginUtils.getPluginsFromExternalJar(pluginJar, moduleType);
    assertNotNull(result);
    assertEquals(2, result.size());
    assertNotNull(result.get(0));
    assertTrue(result.get(0).getCanonicalName()
        .startsWith("com.patriotcoder.testdocussandraplugin.TestNotifierPlugin"));
  }
