...
public class IndexMaintainerHelperTest
{
    ...
    public void testGenerateDocumentUpdateIndexEntriesStatements() throws IndexParseException
    {
        ...
        assertEquals("docussandra", one.getKeyspace());
        assertEquals("UPDATE mydb_mytable_myindexwithonefield SET object = ?, updated_at = ? WHERE bucket = ? AND id = ? AND myindexedfield = ?;", one.preparedStatement().getQueryString());
        BoundStatement two = result.get(1);
        assertNotNull(two);
        for (int i = 0; i < 4; i++)
        {
    ...
    public void testGenerateDocumentUpdateIndexEntriesStatementsWithDataTypes() throws IndexParseException
    {
        ...
        for (int i = 0; i < 11; i++)
        {
            assertTrue(one.isSet(i));// 0 is the blob, 1 is the date, 2 is the UUID, 3 is the doc id, 4-10 are the other indexes
        }
        //check the proper types were set
    ...
    public void testGenerateCQLStatementForUpdate()
    {
        ...
        String expResult = "UPDATE mydb_mytable_myindexwithonefield SET object = ?, updated_at = ? WHERE bucket = ? AND id = ? AND myindexedfield = ?;";
        String result = IndexMaintainerHelper.generateCQLStatementForWhereClauses(IndexMaintainerHelper.ITABLE_UPDATE_CQL, index1);
        assertEquals(expResult, result);
        expResult = "UPDATE mydb_mytable_myindexwithtwofields SET object = ?, updated_at = ? WHERE bucket = ? AND id = ? AND myindexedfield1 = ? AND myindexedfield2 = ?;";
