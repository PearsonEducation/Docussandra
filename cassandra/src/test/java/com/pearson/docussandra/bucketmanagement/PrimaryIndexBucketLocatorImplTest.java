...
public class PrimaryIndexBucketLocatorImplTest {
  ...
  /**
   * Test to convert the double values to long values also checks if the converted values are
  ...
  public void testConvertDoubleToLong() {
    ...
    assertTrue("Function not monotonically increasing.", PrimaryIndexBucketLocatorImpl
        .convertDoubleToLong(1.0) < PrimaryIndexBucketLocatorImpl.convertDoubleToLong(2.0));
    assertTrue("Function not monotonically increasing.", PrimaryIndexBucketLocatorImpl
        .convertDoubleToLong(2.0) < PrimaryIndexBucketLocatorImpl.convertDoubleToLong(3.0));
    assertTrue("Function not monotonically increasing.", PrimaryIndexBucketLocatorImpl
        .convertDoubleToLong(-2.0) < PrimaryIndexBucketLocatorImpl.convertDoubleToLong(-1.0));

    assertTrue("Function not monotonically increasing.",
        PrimaryIndexBucketLocatorImpl.convertDoubleToLong(
            -900000000000.0) < PrimaryIndexBucketLocatorImpl.convertDoubleToLong(-1.0));
  }
  ...
  public void testDateToLongNull() {
    assertNull("the value is not null as expected",
        PrimaryIndexBucketLocatorImpl.convertDateToBucketingLong(null));
  }

  @Test
  public void testConvertObjectToBucketingToken() {
    assertEquals(PrimaryIndexBucketLocatorImpl.convertObjectToBucketingToken(new Boolean(true),
        FieldDataType.BOOLEAN), new Long(1L));
    assertEquals(PrimaryIndexBucketLocatorImpl.convertObjectToBucketingToken(new Boolean(false),
        FieldDataType.BOOLEAN), new Long(0L));
  }
  ...
  /**
   * Test to compare the timestamp values of two different type1 UUIDs the timestamp values of two
   * different type1 UUID's should not be equal
   */
  @Test
  public void testUUIDToLongTimestamp() {
    ...
    assertTrue(timestampOfUuid1 + " is different from timestampOfUuid2",
        !timestampOfUuid11.equals(timestampOfrecentUuid1));
    assertTrue("Timestamp of recent UUID should be greater then the old uuid",
        timestampOfUuid11 < timestampOfrecentUuid1);
  }

  /**
   * Compare the long epoch time values between convertDateToBucketingLong and a java method both
   * the values should be equal, a random hardcoded value is being used
   */
  @Test
  public void testConvertDateToBucketingLong() throws Exception {
    ...
    long time2 = PrimaryIndexBucketLocatorImpl.convertDateToBucketingLong(
        new SimpleDateFormat("MM/dd/yyyy HH:mm:ss z").parse("06/15/2014 22:31:42 GMT"));
  ...
  /**
   * Test to check how the convertIntegerToBucketingLong reacts to null values a return value of
   * null is expected
   */
  @Test
  ...
  /**
   * Test to check the overflowing in the integer should not return a null or end in an exception
   */
  @Test
  ...
  public void testConvertIntegerToBucketingLong() {
    ...
    // assertNearMinLongValue(result);
    assertTrue("Function not monotonically increasing.",
        PrimaryIndexBucketLocatorImpl.convertIntegerToBucketingLong(
            1) < PrimaryIndexBucketLocatorImpl.convertIntegerToBucketingLong(2));
    assertTrue("Function not monotonically increasing.",
        PrimaryIndexBucketLocatorImpl.convertIntegerToBucketingLong(
            2) < PrimaryIndexBucketLocatorImpl.convertIntegerToBucketingLong(3));
    assertTrue("Function not monotonically increasing.",
        PrimaryIndexBucketLocatorImpl.convertIntegerToBucketingLong(
            -2) < PrimaryIndexBucketLocatorImpl.convertIntegerToBucketingLong(-1));
  }

  /**
   * Test to check the return value for double to long conversion null is expected
   */
  @Test
  public void testDoubleToLongNullValues() {
    logger.debug("convert Double to long with null values");
    assertNull("the value is not null as expected ",
        PrimaryIndexBucketLocatorImpl.convertDoubleToLong(null));
  }
  ...
  /**
   * This test is trying to run all the possible strings will different chars also the test is
   * checking the ordering of the long values and tests string which range from 1 char to greater
   * than 8 chars
   */
  @Test
  public void testConvertStringToLong() {
    ...
    testString[9] =
        "wwwwwwwwwwwwwwwwwerwerwerwerweiurhowehriouwheorwemrowiehrmowiehhowieuhwomieurhmwieuhmrwuhemroiwuhemrowiuehrmoiwuehrmowieuhrmoiwuehromiwuehmriuwebifbvwdyfovwdy";
    testString[10] = "a";
    ...
    assertTrue("the string A is supposed to less in value than the string B",
        testLong[0] < testLong[1]);
    assertTrue("the string A is supposed to less in value than the string B",
        testLong[1] < testLong[2]);
    assertTrue("the string A is supposed to less in value than the string B",
        testLong[2] < testLong[3]);
  }
  ...
  public void testConvertGreekStringToLong() {
    ...
    assertTrue("testString[0] should not be equal to testString[1]",
        !testLong[0].equals(testLong[1]));
    assertTrue("testString[1] should not be equal to testString[2]",
        !testLong[1].equals(testLong[2]));
    assertTrue("testString[2] should not be equal to testString[3]",
        !testLong[2].equals(testLong[3]));
  }
  ...
  // }
  /**
   * Test to compare long epoch time values time1(hardcoded) values should always be less than
   * current epoch time
   */
  @Test
  public void testDateToLongCompareTimes() throws Exception {
    ...
    long time1 = PrimaryIndexBucketLocatorImpl.convertDateToBucketingLong(result);
    assertTrue("time1 is not less than timeCurrent" + time1 + "<" + timeCurrent,
        time1 < timeCurrent);
    ...
    assertTrue("current time is not less than future time" + timeCurrent + "<" + timeFuture,
        timeCurrent < timeFuture);
  ...
  public void testBinaryBase64ToLong() {
    ...
    Long binaryBase64Dupl =
        PrimaryIndexBucketLocatorImpl.convertBinaryBase64ToLong(binaryBase64Duplicate);
    assertTrue("First binaryBase64 String should be less than the second one",
        firstBinLongVal < secondBinLongVal);
    assertTrue("Second binaryBase64 String should be less than the third one",
        secondBinLongVal < thirdBinLongVal);
    assertEquals("Duplicate input values should return the same output Long value",
        secondBinLongVal, binaryBase64Dupl);
    Long fourthBinLongVal = PrimaryIndexBucketLocatorImpl.convertBinaryBase64ToLong(binaryBase644);// checks
                                                                                                   // our
                                                                                                   // case
                                                                                                   // when
                                                                                                   // we
                                                                                                   ...
                                                                                                   // 8
                                                                                                   // chars
    assertTrue("Forth binaryBase64 String should be greater than the first one",
        fourthBinLongVal > firstBinLongVal);// 4th should be greater than 1st
  }
  ...
  private void assertNearMaxLongValue(Long in) {
    assertTrue("Value: " + in + " is not close enough to the Long MAX value.",
        in > Long.MAX_VALUE - 100);
  }

  private void assertNearMinLongValue(Long in) {
    assertTrue("Value: " + in + " is not close enough to the Long MIN value.",
        in < Long.MIN_VALUE + 100);
  }

  private void assertNearZeroLongValue(Long in) {
    assertTrue("Value: " + in + " is not close enough to the Long ZERO value.",
        in > -100 || in < -100);
  }

  /**
   * Test of convertTimepointToBucketingLong method, of class SimpleIndexBucketLocatorImpl.
   */
  @Test
  public void testConvertTimepointToBucketingLong() {
    ...
    Long expResult =
        timepointToBeConverted.getTime() - PrimaryIndexBucketGeneratorImpl.TIMEPOINT_MIN;
    // base test
    Long result =
        PrimaryIndexBucketLocatorImpl.convertTimepointToBucketingLong(timepointToBeConverted);
    ...
    assertEquals(PrimaryIndexBucketGeneratorImpl.TIMEPOINT_MAX, result);// make sure it knocks it
                                                                        // back down to 1 Jan 2030
                                                                        // (with offset)
    // date prior to 2000
  ...
  /**
   * Test of convertBinaryBase64ToLong method, of class SimpleIndexBucketLocatorImpl.
   */
  @Test
  @Ignore
  ...
  /**
   * Test of convertStringToBuckectingLong method, of class SimpleIndexBucketLocatorImpl.
   */
  @Test
  @Ignore
