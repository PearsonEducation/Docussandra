...
/**
 * Locates a bucket for an item based on the first few most significant bits of the object.
 */
public class PrimaryIndexBucketLocatorImpl implements BucketLocator {
  ...
  /**
   * Array containing all of our buckets. Outer array is an array of all our buckets by
   * FieldDataType as indexed by FieldDataType.getIndexForDataType(); The inner lists are the actual
   * buckets for each datatype.
   */
  private static List<Long>[] BUCKETS;

  /**
   * Array of our bucket sizes as indexed by FieldDataType.getIndexForDataType();
   */
  // public static int[] BUCKET_SIZES = new int[FieldDataType.getNumberOfDataTypes()];
  /**
   * Method for getting an instance of SimpleIndexBucketLocatorImpl. Will return an existing
   * instance if it exists, will create a new one otherwise. (In other words, it's a singleton.)
   *
  ...
  private PrimaryIndexBucketLocatorImpl() {
    ...
    try {
      ...
      BUCKETS[FieldDataType.TEXT.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/text.buckets");
      BUCKETS[FieldDataType.DATE_TIME.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/date_time.buckets");
      BUCKETS[FieldDataType.TIMEPOINT.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/timepoint.buckets");
      BUCKETS[FieldDataType.DOUBLE.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/double.buckets");
      BUCKETS[FieldDataType.INTEGER.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/integer.buckets");
      BUCKETS[FieldDataType.LONG.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/long.buckets");
      BUCKETS[FieldDataType.BOOLEAN.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/boolean.buckets");
      BUCKETS[FieldDataType.UUID.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/uuid.buckets");
      BUCKETS[FieldDataType.BINARY.getIndexForDataType()] =
          readBucketFromClassPath("/buckets/binary.buckets");
    } catch (IOException e) {
  ...
  /**
   * Protected constructor. Do not call for anything other than testing of changing bucket sizes.
  ...
  public PrimaryIndexBucketLocatorImpl(List<Long> textBuckets, List<Long> dateTimeBuckets,
      ...
      List<Long> uuidBuckets, List<Long> binaryBuckets, List<Long> longBuckets,
      List<Long> timepointBuckets) {
    BUCKETS = new List[FieldDataType.getNumberOfDataTypes()];
  ...
  private Long getClosestBucket(Long bucketingToken, FieldDataType dataType) {
    ...
    // make sure the bucket index isn't negative (it will be negative the closest index if it
    // doesn't hit a bucket exactly)
    if (index < 0) {
   ...
   * @throws IndexParseFieldException if the bucketing object cannot be converted to the specified
   *         dataType.
   */
  @Override
  public Long getBucket(Object bucketingObject, FieldDataType dataType)
    ...
    // if what is being passed in should be of a certian datatype, but has not yet been converted
    // from the string in the json
    Object normalizedObject = bucketingObject;
    if (bucketingObject instanceof String
        && !(dataType.equals(FieldDataType.TEXT) || dataType.equals(FieldDataType.BINARY))) {// if
                                                                                             // it's
                                                                                             // a
                                                                                             // string,
                                                                                             // but
  ...
  /**
   * Converts a string to a proper object of the specified type. Don't use for types that are
   * already Strings.
   *
   ...
   * @throws IndexParseFieldException If the object cannot be converted to the specified type.
   */
  private Object convertStringToOject(String s, FieldDataType dataType)
      throws IndexParseFieldException {
    if (dataType.equals(FieldDataType.BOOLEAN)) {
      return ParseUtils.parseStringAsBoolean(s);
        ...
        || dataType.equals(FieldDataType.TIMEPOINT)) {
   ...
   *
   * @param toBeConverted Object that needs to be converted to a bucketing token.
  ...
  /**
   * This method converts the Date value to its corresponding long value, this method assumes that
   * the date format is the same as the format being returned by the ParseUtils.parseStringAsDate
   *
  ...
  /**
   * This method converts the Timepoint value to its corresponding long value, this method assumes
   * that the date format is the same as the format being returned by the
   * ParseUtils.parseStringAsDate. Note: Any value prior to Y2K (00:00:00 UTC on 1 January 2000)
   * will be bucketed as 0 and any value after 2030 will be bucketed as bucket max. This is done to
   * reduce the number of buckets that are required for this type (by eliminating ranges that are
   * likely to be entirely unused).
   *
  ...
  protected static Long convertTimepointToBucketingLong(Date timepointToBeConverted) {
    ...
    Long toReturn =
        timepointToBeConverted.getTime() - PrimaryIndexBucketGeneratorImpl.TIMEPOINT_MIN; // offset
                                                                                          // from
                                                                                          // y2k
                                                                                          // (year
                                                                                          // 2000
    ...
    if (toReturn < 0l) {
      return 0l;
    } else if (toReturn > PrimaryIndexBucketGeneratorImpl.TIMEPOINT_MAX)// TIMEPOINT_MAX is already
                                                                        // scaled from y2k
    {
  ...
  // private static final double DOUBLE_SCALE_FACTOR = Double.MAX_VALUE / new
  // Double(Long.MAX_VALUE);
  /**
   * Converts a Double to a Long for bucketing purposes. Order of input and output values is
   * preserved as long.
   *
  ...
  /**
   * Converts a binaryBase64 to a Long for bucketing purposes. Order of input and output values is
   * preserved as long.
   *
   * @param binaryBase64 to convert to Long
   * @return a Long value for use in bucket replacement
   */
  protected static Long convertBinaryBase64ToLong(String binaryBase64)
    ...
    if (binaryBase64.length() < 4) {
      throw new IllegalArgumentException(
          "Input is not a valid BinaryBase64 value, minimum length of BinaryBase64 is 4. Enter a valid value");
    }
    ...
    if (binaryBase64.length() >= 8) {
      ...
      for (int i = 0; i < toPad; i++) {
        first8Chars += '\u0000';// not the most efficent, but shouldn't happen often or for very
                                // many chars
      }
    }
  ...
  /**
   * Converts an Integer to a Long for bucketing purposes. Order of integers will be preserved as
   * longs, but not the actual numeric value. This method should <b>only</b> be used for determining
   * bucket placement.
   *
  ...
  /**
   * Converts a string into long values takes the first 8 bytes and uses those to convert them into
   * long, if the string does not contain 8 bytes we pad them with zeros
   *
  ...
  /**
   * Converts a UUID to a Long for bucketing purposes. Order of input and output values is preserved
   * as long.
   *
