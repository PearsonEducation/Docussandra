...
/**
 * Class for generating our bucket lists. This (probably) will NOT be executed by Docussandra it
 * self, but as a separate run prior to actual Docussandra startup. This should only need to be run
 * once (total) per datatype; the results can be saved off and packaged with the app. Writes results
 * to the file system as a CSV with an extension of .buckets.
 *
...
public class PrimaryIndexBucketGeneratorImpl {
   ...
   * Main method.
   *
   * @param args Command line arguments. Should be a single parameter in the form of a
   *        FieldDataType.
   */
  ...
  // prune factors for each data type; if 0; no pruning -- the smaller the prune factor, the more it
  // prunes
  ...
  public final static Long BINARY_MAX = 0x7a7a7a7a7a7a7a7al;// zzzzzzzz -- highest base 64 value we
                                                            // could have
  public final static Long LONG_MAX = Long.MAX_VALUE / 128;
  public final static Long TIMEPOINT_MIN = 946684800000l; // 1 JAN 2000 -- minimum date we will gen
                                                          // buckets for
    ...
    switch (dataType) {
      ...
      default:
        throw new UnsupportedOperationException(
            "DataType: " + dataType.toString() + " is not supported. This is probably a bug.");
    }
  }
  ...
  public PrimaryIndexBucketGeneratorImpl(FieldDataType dataType, int bucketSize)
  ...
  public List<Long> generateBuckets() throws FileNotFoundException, UnsupportedEncodingException {
    logger.info(
        "Generating buckets for: " + dataType.toString() + " with prune factor: " + pruneFactor);
    ...
    if (dataType.equals(FieldDataType.DOUBLE)) {
      logger.info(
          "Special case for double; adding a set of low buckets for high precision on lower numbers.");
   ...
   * @throws UnsupportedEncodingException
   */
  private void writeResults(List<Long> buckets)
      throws FileNotFoundException, UnsupportedEncodingException {
    StringBuilder contentToWrite = new StringBuilder(bucketSize * 8);// ballpark the size
    for (Long l : buckets) {
      contentToWrite.append(l).append(DE);
    }
    Utils.writeFile(contentToWrite.toString(), bucketSize + "bucketsFor" + dataType.toString()
        + "withPruneFactor" + pruneFactor + "MaxSize" + bucketMax + ".buckets");
  }
  ...
  protected List<Long> createBuckets() {
    ...
    for (int bucketPosition = 0; bucketPosition < bucketSize; bucketPosition++)// calculate buckets
                                                                               // from 0 to the size
                                                                               // of this bucket
    {
  ...
  /**
   * Prunes our buckets from the end of the bucket list. Once a pruning start position is
   * established, it prunes chunks of the bucket list based on bucketPruningInteration^2 * 32.
   *
   * @param bucketList Bucket List to prune.
   * @param pruneFactor decimal value (less than 1) that indicates where to start our pruning. Does
   *        not impact pruning after a start point is established. Lower values indicates an earlier
   *        start point (more pruning), higher values indicate a later start point (less pruning).
  ...
  private List<Long> pruneBucket(List<Long> bucketList, double pruneFactor) {
    ...
    while (pruneStartIndex < bucketList.size()) {
      ...
      logger.debug("Pruning " + numToPrune + " buckets at position: " + pruneStartIndex
          + ", bucket list is currently: " + bucketList.size() + " long.");
      // --optinal alternative method for pruning that uses more CPU and less memory; havne't
      // checked on this in a while.
    ...
    logger.debug("Pruned " + numRecordsPruned + " buckets out of " + startSize
        + " for a percent of " + percentPruned);
