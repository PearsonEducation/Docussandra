...
public class IndexCreatedHandler implements EventHandler {
  ...
  public IndexCreatedHandler(IndexRepository indexRepo, IndexStatusRepository indexStatusRepo,
  ...
  public void handle(Object event) throws Exception {
    ...
    // Thread.sleep(1000);//pause for a second to ensure the iTable gets created before proceeding
    ...
    try {
      ...
      QueryResponseWrapper responseWrapper =
          docRepo.readAll(index.getDatabaseName(), index.getTableName(), CHUNK, offset);
      boolean hasMore;
      ...
      while (hasMore) {
        for (Document toIndex : responseWrapper) {
          try {
            // actually index here
            BoundStatement statement = IndexMaintainerHelper
                .generateDocumentCreateIndexEntryStatement(indexStatusRepo.getSession(), index,
                    toIndex, PrimaryIndexBucketLocatorImpl.getInstance());
            if (statement != null) {
            ...
            recordsCompleted++;// we will still call this record "complete" for the sake of time
                               // calculation, and percent done
            ...
            errors.add(e.toString() + " In document: " + toIndex.toString());// may want to reduce
                                                                             // the verbosity here
                                                                             // eventually -- or
                                                                             // break some meta-data
                                                                             // out into columns
      ...
      if (status != null) {
        ...
        status.setFatalError(
            errorMessage + " Please contact a system administrator to resolve this issue.");
        status.setStatusLastUpdatedAt(new Date());
        indexStatusRepo.update(status);
      }
      throw e;
