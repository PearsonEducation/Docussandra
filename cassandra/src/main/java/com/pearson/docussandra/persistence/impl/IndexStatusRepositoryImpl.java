 ...
 */
public class IndexStatusRepositoryImpl extends AbstractCRUDRepository<IndexCreatedEvent>
    implements IndexStatusRepository {
  ...
  private static final String DELETE_FROM_NOT_DONE =
      "delete from " + Tables.BY_NOT_DONE + IDENTITY_CQL;
  private static final String CREATE_CQL = "insert into " + Tables.BY_ID + " (" + Columns.ID + ", "
      + Columns.DATABASE + ", " + Columns.TABLE + ", " + Columns.INDEX_NAME + ", "
      + Columns.RECORDS_COMPLETED + ", " + Columns.TOTAL_RECORDS + ", " + Columns.STARTED_AT + ", "
      + Columns.UPDATED_AT + ", " + Columns.ERRORS + ", " + Columns.FATAL_ERROR
      + ") values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) IF NOT EXISTS";
  private static final String READ_CQL = "select * from " + Tables.BY_ID + IDENTITY_CQL;
  private static final String UPDATE_CQL =
      "update " + Tables.BY_ID + " set " + Columns.RECORDS_COMPLETED + " = ?, " + Columns.UPDATED_AT
          + " = ?, " + Columns.ERRORS + " = ?, " + Columns.FATAL_ERROR + " = ?" + IDENTITY_CQL;
  private static final String MARK_INDEXING_CQL =
      "insert into " + Tables.BY_NOT_DONE + "(" + Columns.ID + ") values (?) IF NOT EXISTS";

  private static final String READ_ALL_CQL = "select * from " + Tables.BY_ID;
  private static final String READ_ALL_CURRENTLY_INDEXING_CQL =
      "select * from " + Tables.BY_NOT_DONE;
  private static final String IS_CURRENTLY_INDEXING_CQL =
      "select count(*) from " + Tables.BY_NOT_DONE + IDENTITY_CQL;// records that are currently
                                                                  // indexing (not yet done)
  ...
  protected void initialize() {
    ...
    readAllCurrentlyIndexingStmt = PreparedStatementFactory
        .getPreparedStatement(READ_ALL_CURRENTLY_INDEXING_CQL, getSession());
    deleteFromNotDoneStmt =
        PreparedStatementFactory.getPreparedStatement(DELETE_FROM_NOT_DONE, getSession());
    markIndexingStmt =
        PreparedStatementFactory.getPreparedStatement(MARK_INDEXING_CQL, getSession());
    isCurrentlyIndexingStmt =
        PreparedStatementFactory.getPreparedStatement(IS_CURRENTLY_INDEXING_CQL, getSession());
  }
  ...
  /**
   * Updates the status for an IndexCreatedEvent in the database. Take note: only a few getFields
   * are updatable: numberOfRecordsCompleted, statusLastUpdatedAt, and error. This is a logical
   * decision; there should not be a reason to update any other getFields. This will also mark the
  ...
  public void delete(Identifier id) {
    throw new UnsupportedOperationException(
        "Not a valid call for this class. IndexStatuses cannot be deleted.");
  }
  ...
  /**
   * Gets all IndexCreatedEvents that are currently indexing. This method is preferred to readAll().
   * This provides a sense of the indexing load on the database.
   *
  ...
  /**
   * Determines if a index is currently indexing or not. Not presently used, but the method should
   * work.
   *
   * @param id
   * @return
  ...
  private void bindCreate(BoundStatement bs, IndexCreatedEvent entity) {
    bs.bind(entity.getUuid(), entity.getIndex().getDatabaseName(), entity.getIndex().getTableName(),
        entity.getIndex().getName(), entity.getRecordsCompleted(), entity.getTotalRecords(),
        entity.getDateStarted(), entity.getStatusLastUpdatedAt(), entity.getErrors(),
        entity.getFatalError());
  }

  private void bindUpdate(BoundStatement bs, IndexCreatedEvent entity) {
    bs.bind(entity.getRecordsCompleted(), entity.getStatusLastUpdatedAt(), entity.getErrors(),
        entity.getFatalError(), entity.getUuid());
  }
  ...
  protected IndexCreatedEvent marshalRow(Row row) {
    ...
    } catch (ItemNotFoundException e)// this should only happen in tests that do not have full test
                                     // data established; errors will be evident if this happens in
                                     // the actual app
    {
      toUse = index;
    }
    IndexCreatedEvent i = new IndexCreatedEvent(row.getUUID(Columns.ID),
        row.getDate(Columns.STARTED_AT), row.getDate(Columns.UPDATED_AT), toUse,
        row.getLong(Columns.TOTAL_RECORDS), row.getLong(Columns.RECORDS_COMPLETED));
