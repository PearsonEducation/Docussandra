...
public class QueryRepositoryImpl implements QueryRepository {
  ...
  private static final String QUERY_CQL_LIMIT = "select * from %s where bucket = ? AND %s LIMIT %s";// we
                                                                                                    // use
                                                                                                    // the
                                                                                                    // limit
                                                                                                    // as
  ...
  private BoundStatement generateQueryStatement(ParsedQuery query, long maxIndex)
    ...
    {
      finalQuery = String.format(QUERY_CQL, query.getITable(),
          query.getWhereClause().getBoundStatementSyntax());
    } else // with a limit
    {
      finalQuery = String.format(QUERY_CQL_LIMIT, query.getITable(),
          query.getWhereClause().getBoundStatementSyntax(), maxIndex);
    }
    // run query
    ...
    Long bucket;
    try {
      bucket = PrimaryIndexBucketLocatorImpl.getInstance().getBucket(
          query.getWhereClause().getValues().get(0), query.getIndex().getFields().get(0).getType());
   ...
   */
  @Override
  public QueryResponseWrapper query(ParsedQuery query, int limit, long offset)
      throws IndexParseException {
    // run the query
    long maxIndex = offset + limit;
    ResultSet results = session.execute(generateQueryStatement(query, maxIndex + 1));// we do one
                                                                                     // plus here so
                                                                                     // we know if
                                                                                     // there are
                                                                                     // additional
