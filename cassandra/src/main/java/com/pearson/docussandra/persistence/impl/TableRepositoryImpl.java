...
public class TableRepositoryImpl extends AbstractCRUDRepository<Table> implements TableRepository {
  ...
  private static final String IDENTITY_CQL =
      " where " + Columns.DATABASE + " = ? and " + Columns.NAME + " = ?";
  private static final String EXISTENCE_CQL = "select count(*) from %s" + IDENTITY_CQL;
  private static final String CREATE_CQL = "insert into %s (%s, " + Columns.DATABASE + ", "
      + Columns.DESCRIPTION + ", " + Columns.CREATED_AT + ", " + Columns.UPDATED_AT
  ...
  private static final String UPDATE_CQL = "update %s set " + Columns.DESCRIPTION + " = ?, "
      + Columns.UPDATED_AT + " = ?" + IDENTITY_CQL;
  ...
  private static final String READ_ALL_COUNT_CQL =
      "select count(*) from %s where " + Columns.DATABASE + " = ?";
  private static final String READ_COUNT_TABLE_SIZE_CQL = "select count(*) from %s";

  private static final String CREATE_DOC_TABLE_CQL =
      "create table %s" + " (id uuid, object blob, " + Columns.CREATED_AT + " timestamp, "
          + Columns.UPDATED_AT + " timestamp," + " primary key ((id), " + Columns.UPDATED_AT + "))"
  ...
  protected void initialize() {
    ...
    updateStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(UPDATE_CQL, getTable()), getSession());
  ...
  public void delete(Identifier id) {
    ...
    Statement s = new SimpleStatement(
        String.format(DROP_DOC_TABLE_CQL, id.getDatabaseName() + "_" + id.getTableName()));
    getSession().execute(s);
  ...
  public long countTableSize(String database, String tableName) {
    PreparedStatement readCountTableSizeStmt = PreparedStatementFactory.getPreparedStatement(
        String.format(READ_COUNT_TABLE_SIZE_CQL, database + "_" + tableName), getSession());
  ...
  private void bindCreate(BoundStatement bs, Table entity) {
    bs.bind(entity.getName(), entity.getDatabase().getName(), entity.getDescription(),
        entity.getCreatedAt(), entity.getUpdatedAt());
  }

  private void bindUpdate(BoundStatement bs, Table entity) {
    bs.bind(entity.getDescription(), entity.getUpdatedAt(), entity.getDatabase().getName(),
        entity.getName());
  }
