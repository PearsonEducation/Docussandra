...
public class IndexRepositoryImpl extends AbstractCRUDRepository<Index> implements IndexRepository {
  ...
  private static final String CREATE_CQL =
      "insert into %s (%s, db_name, tbl_name, is_unique, fields, fields_type, only, is_active, created_at, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) IF NOT EXISTS";
  ...
  private static final String READ_ALL_COUNT_CQL =
      "select count(*) from %s where db_name = ? and tbl_name = ?";
  ...
  protected void initialize() {
    existStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(EXISTENCE_CQL, getTable()), getSession());
    readStmt = PreparedStatementFactory.getPreparedStatement(String.format(READ_CQL, getTable()),
        getSession());
    createStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(CREATE_CQL, getTable(), Columns.NAME), getSession());
    deleteStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(DELETE_CQL, getTable()), getSession());
    markActiveStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(MARK_ACTIVE_CQL, getTable()), getSession());
    readAllStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(READ_ALL_CQL, getTable()), getSession());
    readAllCountStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(READ_ALL_COUNT_CQL, getTable()), getSession());
  }
  ...
  public Index create(Index entity) {
    ...
    // -----check to see if it is correct, suggest the user delete and try again if it's not --
    // probably
    // -----automatically re-index; hard to actually do, it would need a different name if the index
    // was in use -- probably not
    // -----do nothing -- maybe?
    return entity;
  }
  ...
  public Index update(Index entity) {
    throw new UnsupportedOperationException(
        "Updates are not supported on indices; create a new one and delete the old one if you would like this functionality.");
  }
  ...
  public void delete(Identifier id) {
    ...
    // TODO: delete the index status (IndexCreationEvents) (or at least mark it permantly
    // inactive/disabled with a new timestamp, ensuring that /index_status won't return it) and halt
    // any active indexing to save processor time -- issue
    // https://github.com/PearsonEducation/Docussandra/issues/3
  }
  ...
  public List<Index> readAllCached(Identifier id) {
    ...
    if (e == null || e.getObjectValue() == null)// if its not set, or set, but null, re-read
    {
      ...
      if (all != null && !all.isEmpty())// don't store empty or null index lists; could cause
                                        // problems if an index gets created and we are not treating
                                        // it as created
      {
  ...
  private void cascadeDelete(IndexIdentifier id) {
    logger.info("Cleaning up ITables for index: " + id.getDatabaseName() + "/" + id.getTableName()
        + "/" + id.getIndexName());
  ...
  private void bindCreate(BoundStatement bs, Index entity) {
    bs.bind(entity.getName(), entity.getDatabaseName(), entity.getTableName(), entity.isUnique(),
        entity.getFieldsValues(), entity.getFieldsTypes(), entity.getIncludeOnly(),
