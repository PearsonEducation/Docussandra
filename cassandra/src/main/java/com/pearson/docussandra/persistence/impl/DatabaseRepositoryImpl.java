...
public class DatabaseRepositoryImpl extends AbstractCRUDRepository<Database>
  ...
  private static final String CREATE_CQL =
      "insert into %s (%s, description, created_at, updated_at) values (?, ?, ?, ?) IF NOT EXISTS";
  private static final String UPDATE_CQL =
      "update %s set description = ?, updated_at = ? where %s = ?";
  ...
  protected void initializeStatements() {
    ...
    updateStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(UPDATE_CQL, getTable(), Columns.NAME), getSession());
    ...
    existStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(EXISTENCE_CQL, getTable(), Columns.NAME), getSession());
    readStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(READ_CQL, getTable(), Columns.NAME), getSession());
    deleteStmt = PreparedStatementFactory
        .getPreparedStatement(String.format(DELETE_CQL, getTable(), Columns.NAME), getSession());
  }
  ...
  private void bindCreate(BoundStatement bs, Database entity) {
    bs.bind(entity.getName(), entity.getDescription(), entity.getCreatedAt(),
        entity.getUpdatedAt());
  }

  private void bindUpdate(BoundStatement bs, Database entity) {
    bs.bind(entity.getDescription(), entity.getUpdatedAt(), entity.getName());
  }
