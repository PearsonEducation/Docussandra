...
//        }
     ...
     *
     * @param dropDb Start the database dropDb; will <b>DROP THE EXISTING
     * DATABASE</b>. Do not pass true except for testing.
     * @param replicationString Replication string to use when creating the
     * keyspace.
     * @param session Cassandra session to use to connect in order to execute
     * the db create.
     */
    public static void initDatabase(boolean dropDb, String replicationString, Session session)
    {
        //check that the replication string looks at least a little valid so we don't SQL inject that easily
        if (replicationString.startsWith("{") && replicationString.endsWith("}") && replicationString.contains("Strategy") && replicationString.contains("'class'"))
        {
            logger.info("Initing Docussandra Cassandra Database with replication factor: \"" + replicationString + "\".");
        } else
        {
            throw new IllegalArgumentException("Replication String: \"" + replicationString + "\" does not appear to be valid.");
        }
        if (dropDb)
        {
            //Fix with?: https://github.com/PearsonEducation/Docussandra/issues/1
            logger.warn("WARNING: DROPPING EXISTING DOCUSSANDRA database! You have 5 seconds to kill this process before the data will be dropped.");
            try
            {
                Thread.sleep(5000);
        ...
        executeWithLog(session, "create keyspace IF NOT EXISTS docussandra with replication = " + replicationString + ";");
        executeWithLog(session, "use docussandra;");
        executeWithLog(session, "create table IF NOT EXISTS sys_meta (\n"
                + "	id text,\n"
                + "	version text,\n"
        ...
        executeWithLog(session, "create table IF NOT EXISTS sys_db (\n"
    ...
    /**
     * Creates the initial Docussandra database with a replication factor of 1
     * for local deploys and testing.
     *
     * @param dropDb Start the database dropDb; will <b>DROP THE EXISTING
     * DATABASE</b>. Do not pass true except for testing.
     * @param session Cassandra session to use to connect in order to execute
    ...
    public static void setField(String value, IndexField fieldData, BoundStatement bs, int index) throws IndexParseException
    {
        try
        {
            ...
            } else if (fieldData.getType().equals(FieldDataType.DATE_TIME) || fieldData.getType().equals(FieldDataType.TIMEPOINT))
            {
    ...
    /**
     * Writes a file to the file system. Used mainly for testing and some utils.
     ...
     */
    public static void writeFile(String toWrite, String fileName) throws FileNotFoundException, UnsupportedEncodingException
    {
    ...
    /**
     * Method to convert a 1 row file to long arraylist values fileName needs to
     * be the entire location of the file example :
     * src/test/resources/bucketsDate.csv
     */
    public static List<Long> traverse1RowFile(String fileName) throws IOException
    {
        ...
        for (int i = 0; i < splitString.length; i++)
        {
            ...
            {
                logger.error("Could not parse token: " + splitString[i] + " from file: " + fileName + " at index: " + i, e);
                throw e;
            }
        }        
