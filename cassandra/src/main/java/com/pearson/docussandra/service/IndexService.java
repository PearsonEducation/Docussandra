...
public class IndexService {
   ...
   * Constructor.
   *
   * @param tableRepository Table repository to use for interacting with tables.
   * @param indexRepository Index repository to use for interacting with indexes.
   * @param status IndexStatus repository to use for interacting with index statuses.
   */
  public IndexService(TableRepository tableRepository, IndexRepository indexRepository,
      IndexStatusRepository status) {
    super();
  ...
  /**
   * Creates an index. Index will be created synchronously, but it will be populated asynchronously,
   * and it will not be "active" (meaning it can't be used for querying) until indexing is complete.
   * Inactive indexes will still be populated by NEW records that are being inserted.
   *
   * @param index Index to create.
   * @return An IndexCreatedEvent that contains the index and some metadata about it's creation
   *         status.
   */
  public IndexCreatedEvent create(Index index) {
    ...
    index.setActive(false);// we default to not active when being created; we don't allow the user
                           // to change this; only the app can change this
  ...
  /**
   * Gets the status of an index creation event. Allows a user to check on the status of the
   * indexing for an index.
   *
  ...
  /**
   * Gets statuses for ALL pending index creations. Allows users and admins to check the indexing
   * load on the system.
   *
