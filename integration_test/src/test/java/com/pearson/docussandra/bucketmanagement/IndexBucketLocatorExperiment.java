...
/**
 * Class for experimenting with bucketing concepts. Written as a test file,
 * however these methods were written to be run ad-hoc, one at a time to
 * experiment with different bucketing settings. These are not really tests at
 * all -- no asserts. As such, they are all marked as ignored until bucketing
 * changes are needed again.
 *
...
public class IndexBucketLocatorExperiment
{
    ...
    {
        //SimpleIndexBucketLocatorImpl locator = new SimpleIndexBucketLocatorImpl(null, Integer.MAX_VALUE/64, null, null, null, null, null, null);
...
//        writeFile(RunStats.getCSVHeader() + csv.toString(), "integer.csv");
    ...
    /**
     * checks the Bucket Distribution for the common Integer values prints out
     * the run time
     *
    ...
    public void testGetBucketForIntegers() throws Exception
    {
        ...
        for (int numBuckets = by; numBuckets < max; numBuckets += by)
        {
            ...
            Utils.writeFile(res.printDistStatsAsCSV(), "integersWithStatsWith" + numBuckets + "buckets.csv");
        }
        Utils.writeFile(RunStats.getCSVHeader() + csv.toString(), "integer.csv");
    }

    /**
     * checks the Bucket Distribution for the common string values prints out
    ...
    public void testGetBucketForGreekString() throws FileNotFoundException, Exception
    {
        ...
        for (int numBuckets = 25; numBuckets < max; numBuckets += by)
        {
            ...
            if (numBuckets % 1000 == 0)
            {
                ...
                Utils.writeFile(res.printDistStatsAsCSV(), "greekStringStatsWith" + numBuckets + "buckets.csv");
            }
        }
    ...
    /**
     * Test to check the Bucket Distribution for the past and future 100 year
     * Date values A random Date value per day is considered prints out
     * runtime,maxBucketValue, avgNumberOfItems, standardDeviation
     *
    ...
    public void testGetBucketForRecentDates() throws Exception
    {
        ...
        for (int numBuckets = by; numBuckets < max; numBuckets += by)
        {
            ...
            if (numBuckets % 1000000 == 0)
            {
                ...
                Utils.writeFile(res.printDistStatsAsCSV(), "recentDatesStatsWith" + numBuckets + "buckets.csv");
            }
        }
    ...
    /**
     * Test to check the Bucket Distribution for the past and future one week
     * date per minute values prints out runtime,maxBucketValue,
    ...
    public void testGetBucketPerRecentDateForMinute() throws Exception
    {
        ...
        for (int numBuckets = by; numBuckets < max; numBuckets += by)
        {
            RunStats res = BucketCSV.delegateGetBucketPerRecentDateForMinute(numBuckets, FieldDataType.DATE_TIME);
            csv.append(res.toCSVRow());
            if (numBuckets % 100000 == 0)
            {
                Utils.writeFile(res.printDistAsCSV(), "recentDateForMinuteWith" + numBuckets + "buckets.csv");
                Utils.writeFile(res.printDistStatsAsCSV(), "recentDateStatsForMinuteWith" + numBuckets + "buckets.csv");
            }
        }
    ...
    public void testConvertDateToBucketingLong() throws Exception
    {
        ...
        for (int i = 0; i < 1460000; i++)	// There are approximately 1460000 days from 0015 year to 4015 year
        {
            ...
            csv.append(shortFormat.format(date)).append(",").append(date.getTime()).append(",").append(bucketingToken).append("\n");
    ...
    public RunStats delegateGetBucketForRecentDates(int buckets) throws Exception
    {
        ...
        PrimaryIndexBucketGeneratorImpl ibg = new PrimaryIndexBucketGeneratorImpl(FieldDataType.DATE_TIME, buckets);
        PrimaryIndexBucketLocatorImpl locator = new PrimaryIndexBucketLocatorImpl(null, ibg.generateBuckets(), null, null, null, null, null, null, null);
        ...
        RunStats runStats = new RunStats("BucketForRecentDates", FieldDataType.DATE_TIME, runTime, stats, hm);
        logger.debug(runStats.toString());
        return runStats;
    }

    /**
     * Test to check the Bucket Distribution for the past and future 2000 year
     * Date values A random Date value per day is considered prints out
     * runtime,maxBucketValue, avgNumberOfItems, standardDeviation
     *
     * @throws ParseException
    ...
    /**
     * checks the Bucket Distribution for four thousand year Date values A
     * random Date value per day is considered
     *
     * @throws IndexParseFieldException
     */
    ...
    /**
     * checks the Bucket Distribution for the common English and Greek string
     * values prints out runtime,maxBucketValue, avgNumberOfItems,
     * standardDeviation
     *
     * @throws FileNotFoundException
     */
    ...
    public void testGetBucketForGreekAndEnglishString() throws FileNotFoundException, Exception
    {
        ...
        int by = 25;
        StringBuilder csv = new StringBuilder(20 * (max / by));
        for (int numBuckets = by; numBuckets < max; numBuckets += by)// try bucket values from one to may by the value of "by"
        {
            ...
            if (numBuckets % 10000 == 0)
            {
                ...
                Utils.writeFile(res.printDistStatsAsCSV(), "greekAndEnglishStatsWith" + numBuckets + "buckets.csv");
            }
        }
        Utils.writeFile(RunStats.getCSVHeader() + csv.toString(), "string.csv");
    }
    ...
    /**
     * checks the Bucket Distribution for the double values prints out the run
     * time
     *
     * @throws IndexParseFieldException
     */
    ...
    /**
     * checks the Bucket Distribution for the UUID values prints out the run
     * time
     *
     * @throws IndexParseFieldException
     */
    ...
    public void testGetBucketForUuids() throws Exception
    {
        ...
        int by = 25;
        StringBuilder csv = new StringBuilder(20 * (max / by));
        for (int numBuckets = 1; numBuckets < max; numBuckets += by)// try bucket values from one to may by the value of "by"
        {
    ...
    /**
     * checks the Bucket Distribution for the Long values prints out the run
     * time
     *
     * @throws IndexParseFieldException
     */
    ...
    /**
     * checks the Bucket Distribution for the Boolean values prints out the run
     * time
     *
     * @throws IndexParseFieldException
     */
